...
import runnerAxios from './runnerAxios'
...
class Runner {
  ...
  sendRequest(step) {
    return new Promise((resolve, reject) => {
      ...
      try {
        if (step.headers) {
          headersData = json.parse(step.headers)
        }
      } catch (err) {
        reject(err)
        return
      ...
      runnerAxios({
        ...
        headers: {
          'Content-Type':
            step.body_type && step.body_type
              ? this.getAcceptTypes(step.content_type)
              : undefined,
          ...headersData,
        },
      })
  ...
  prepareStepData(step) {
    ...
    if (step.body) {
      const variablesInBody = [
        ...String(step.body).matchAll(variablePattern),
      ].map((match) => ({
        key: match[1],
    ...
    if (step.headers) {
      const variablesInBody = [
        ...String(step.headers).matchAll(variablePattern),
      ].map((match) => ({
        key: match[1],
        withBrackets: match[0],
      }))
  ...
  getVariableFromString(string) {
    ...
    if (parts[0] === 'PREV') {
      ...
      if (
        typeof mainObject === 'string' ||
        typeof mainObject === 'number' ||
        typeof mainObject === 'boolean'
      ) {
        return mainObject
      }
      return ''

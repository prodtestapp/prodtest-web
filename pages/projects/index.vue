<template>
  <div>
    ...
    <ul role="list" class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      ...
      />
      <div class="outline-0 relative flex flex-col justify-center align-center cursor-pointer w-full border-2 border-gray-300 border-dashed rounded-lg p-12 text-center hover:border-gray-400 focus:outline-none focus:ring-2" @click='showCreateModal'>
    ...
    >
      <form class='px-2 py-2' @submit.prevent="addProject">
        <div>
          <label for="create-project-name" class="block text-sm font-medium text-gray-700">{{ $t('Project Name') }}</label>
          <div class="mt-1">
            <input id="create-project-name" v-model='createProject.name' name="project-name" type="text" required="" :class='{"border-red-300": editValidationErrors.name}' class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
          </div>
...
export default {
  ...
  methods: {
    ...
    updateProject(){
      this.$axios.put(`projects/${this.editProject.id}`, {name: this.editProject.name}).then(res => {
    ...
    addProject(){
      this.$axios.post('projects', {name: this.createProject.name}).then(res => {

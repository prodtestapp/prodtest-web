      ...
      <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
        <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            <draggable v-model="steps" tag="table" class="min-w-full divide-y divide-gray-200" handle=".handle" @change='onChangeStepsOrder'>
              <tbody v-for='step in steps' :key='`step-${step.id}`' class="bg-white divide-y divide-gray-200">
                <tr>
                  ...
                  <button type="button" class="-ml-px relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500" @click.prevent='expandStep(step)'>
                    <span class="sr-only">{{$t('Toggle')}}</span>
                    <svg v-if='expandedStepId === step.id' class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <tr v-if='expandedStepId === step.id'>
                  <td colspan='7'>
                    <div class='my-6 flex justify-center'>data</div>
                  </td>
                </tr>
              ...
              <thead slot='header' class="bg-gray-50">
              <tr>
